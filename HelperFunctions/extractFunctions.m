...
while ( ~feof(fr) )
    ...
    isClass = regexpi(l, '^[^%\s]*classdef[\s\S]*');
    ...
    if ( ~isempty( isClass ) )
        ...
        cname = regexpi(l, 'classdef[\s]([\S]+)\s.*', 'tokens');
        
        % If the file was unable to be opened for some reason, throw an
        % error and close the reading file
        if ( length(cname{1}) ~= 1)
            fclose(fr);
            error(['Error parsing classdef line "', l, '"']);
        end
        
        % The first matching group will contain the filename
        cname = cname{1}{1};
        ...
        if (fw == -1)
            fclose(fr);
            error(['Unable to open file ', cname]);
        end
    end
